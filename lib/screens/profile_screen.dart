            ...
            actions: [
              IconButton(
                ...
                onPressed: () {
                  ...
                  p.setThemeMode(Theme.of(context).brightness == Brightness.dark
                      ? ThemeMode.light : ThemeMode.dark);
                },
              ),
              IconButton(icon: const Icon(Icons.edit_outlined, color: Colors.white, size: 20),
                  onPressed: () => _showEdit(context, user)),
              IconButton(icon: const Icon(Icons.logout_rounded, color: Colors.white70, size: 20),
                    ...
                    child: Column(mainAxisAlignment: MainAxisAlignment.end, children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.15), blurRadius: 18)],
                        ),
                        child: Center(child: Text(
                          user.firstName.isNotEmpty ? user.firstName[0].toUpperCase() : '?',
                          style: GoogleFonts.sora(fontSize: 30, fontWeight: FontWeight.w900, color: Colors.white),
                        )),
                      ),
                      const SizedBox(height: 10),
                      Text(user.fullName,
                          style: GoogleFonts.sora(fontSize: 19, fontWeight: FontWeight.w800, color: Colors.white, letterSpacing: -0.3)),
                      const SizedBox(height: 4),
                      Text('${user.grade} · ${user.school}',
                          style: GoogleFonts.nunito(color: Colors.white.withValues(alpha: 0.75), fontSize: 12.5),
                      ...
                      Container(
                        ...
                        child: Text(user.rankTitle,
                            style: GoogleFonts.sora(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w700)),
                      ),
                    ]),
                  )),
                ]),
      ...
      builder: (_) => StatefulBuilder(builder: (ctx, set) => Container(
        ...
        child: Column(children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 22),
              child: Text('Profilni tahrirlash', style: Theme.of(context).textTheme.titleLarge)),
          const SizedBox(height: 18),
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  _Label2('Ism'), const SizedBox(height: 7),
                  TextField(controller: fc, style: GoogleFonts.nunito(color: c.textPrimary, fontWeight: FontWeight.w600),
                      decoration: const InputDecoration(hintText: 'Ism')),
                ])),
                const SizedBox(width: 12),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  _Label2('Familiya'), const SizedBox(height: 7),
                  TextField(controller: lc, style: GoogleFonts.nunito(color: c.textPrimary, fontWeight: FontWeight.w600),
                      decoration: const InputDecoration(hintText: 'Familiya')),
                ])),
              ]),
              const SizedBox(height: 12),
              _Label2('Maktab'), const SizedBox(height: 7),
              TextField(controller: sc, style: GoogleFonts.nunito(color: c.textPrimary, fontWeight: FontWeight.w600),
                  decoration: const InputDecoration(hintText: 'Maktab')),
              const SizedBox(height: 12),
              ...
              SizedBox(width: double.infinity, child: ElevatedButton(
                onPressed: () async {
                  await context.read<AppProvider>().updateProfile(
                      firstName: fc.text.trim(), lastName: lc.text.trim(),
                      school: sc.text.trim(), grade: grade);
    ...
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      actions: [
        TextButton(onPressed: () => Navigator.pop(context),
            child: Text('Bekor qilish', style: GoogleFonts.sora(color: c.textMuted, fontWeight: FontWeight.w600))),
        ElevatedButton(
          onPressed: () async {
...
class _StatsGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final c = context.colors;
    return Row(children: [
      Expanded(child: _SBox(icon: Icons.bolt_rounded, val: '${user.totalScore}', lbl: 'Ball', color: c.primary)),
      const SizedBox(width: 8),
      Expanded(child: _SBox(icon: Icons.star_rounded, val: '${user.totalStars}', lbl: 'Yulduz', color: c.starColor)),
      const SizedBox(width: 8),
      Expanded(child: _SBox(icon: Icons.bar_chart_rounded, val: rank > 0 ? '#$rank' : '-', lbl: 'O\'rin', color: c.accentOrange)),
      const SizedBox(width: 8),
      Expanded(child: _SBox(icon: Icons.workspace_premium_rounded, val: '${user.certificates.length}', lbl: 'Sert.', color: c.accentPink)),
    ]);
  }
}
...
class _SBox extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(children: [
      ...
      const SizedBox(height: 4),
      Text(val, style: GoogleFonts.sora(fontSize: 13, fontWeight: FontWeight.w800, color: color)),
      Text(lbl, style: GoogleFonts.nunito(fontSize: 9, color: context.colors.textMuted), textAlign: TextAlign.center, maxLines: 1),
    ]),
  );
}
    ...
    return Container(
      ...
      child: Row(children: [
        CircularPercentIndicator(
          ...
          center: Text('${(progress * 100).round()}%', style: GoogleFonts.sora(color: c.primary, fontSize: 11, fontWeight: FontWeight.w800)),
          progressColor: c.primary, backgroundColor: c.cardBorder,
        ),
        const SizedBox(width: 16),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text("O'zlashtirish", style: GoogleFonts.sora(color: c.textPrimary, fontWeight: FontWeight.w700, fontSize: 13)),
          const SizedBox(height: 4),
          Text('$completed/$total dars tugallandi', style: GoogleFonts.nunito(color: c.textMuted, fontSize: 11)),
          const SizedBox(height: 8),
          ClipRRect(borderRadius: BorderRadius.circular(5), child: LinearProgressIndicator(value: progress.clamp(0.0, 1.0), minHeight: 6)),
        ])),
      ]),
    );
  }
...
class _Achievements extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (user.certificates.isEmpty) {
      return Container(width: double.infinity, padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: c.cardBg, borderRadius: BorderRadius.circular(16), border: Border.all(color: c.cardBorder)), child: Text("Hali sertifikatlar yo'q", style: GoogleFonts.nunito(color: c.textMuted), textAlign: TextAlign.center));
    }

    return Column(children: user.certificates.map((certId) {
      ...
      return Container(
        ...
        child: Row(children: [
          Icon(Icons.workspace_premium_rounded, color: c.gold, size: 24),
          const SizedBox(width: 12),
          Expanded(child: Text(lesson.title, style: GoogleFonts.sora(fontWeight: FontWeight.w700, fontSize: 13))),
          Icon(Icons.verified_rounded, color: c.gold, size: 20),
        ]),
      );
    }).toList());
  }
...
class _CompletedList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: user.completedLessons.map((id) {
      ...
      return Container(margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: c.cardBg, borderRadius: BorderRadius.circular(14), border: Border.all(color: c.cardBorder)), child: Row(children: [Text(lesson.emoji, style: const TextStyle(fontSize: 18)), const SizedBox(width: 10), Expanded(child: Text(lesson.title, style: GoogleFonts.sora(fontSize: 12, fontWeight: FontWeight.w700))), Text('$score', style: GoogleFonts.sora(fontSize: 11, color: c.textMuted, fontWeight: FontWeight.w700))]));
    }).toList());
  }
}
...
class _InfoCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(children: [
        _IR(Icons.person_rounded, 'Ism', user.fullName),
        _IR(Icons.school_rounded, 'Maktab', user.school),
        _IR(Icons.class_rounded, 'Sinf', user.grade, last: true),
      ]),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Row(children: [
        ...
        Expanded(child: Text(val, style: GoogleFonts.nunito(fontSize: 12, fontWeight: FontWeight.w700))),
      ]),
    );
  }
}
